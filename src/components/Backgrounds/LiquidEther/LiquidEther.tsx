...
export interface LiquidEtherProps {
  ...
  /** Attach listeners to window (true) for interaction when overlay has pointer-events-none */
...
// Golden theme palette (dark to bright) matching site accent
const defaultColors = ['#ca8a04', '#eab308', '#fbbf24', '#fde047'];
...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      container: HTMLElement | null = null;
      target: HTMLElement | Window | null = null;
      isHoverInside = false;
      ...
      init(container: HTMLElement, useGlobal: boolean) {
        this.container = container;
        this.target = useGlobal ? window : container;
        const tgt: any = this.target;
        tgt.addEventListener('mousemove', this._onMouseMove);
        tgt.addEventListener('touchstart', this._onTouchStart, { passive: true });
        tgt.addEventListener('touchmove', this._onTouchMove, { passive: true });
        if (!useGlobal) {
        ...
        tgt.addEventListener('touchend', this._onTouchEnd);
      }
      dispose() {
        const tgt: any = this.target || this.container;
        if (!tgt) return;
        ...
        tgt.removeEventListener('mouseenter', this._onMouseEnter);
        tgt.removeEventListener('mouseleave', this._onMouseLeave);
...
`;
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: any) {
        ...
        Mouse.init(props.$wrapper, !!props.globalEvents);
